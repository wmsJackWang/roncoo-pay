...
@Controller
@RequestMapping(value = "/WeChatJsapiPay")
public class WeChatJsapiPayController extends BaseController {

	private static final Logger logger = LoggerFactory.getLogger(WeChatJsapiPayController.class);
	...
	public String pay(Model model, HttpServletRequest httpServletRequest) {
		...
		String openId = getString_UrlDecode_UTF8("openId");//传入openid
		logger.info("openid:"+openId);
		payOrder.put("openId", openId);
		...
		logger.info("微信JSAPI支付,接收参数:{}", payOrder.toJSONString());
		...
		// 验证签名数据
		// boolean verifyFlag = XXPayUtil.verifyPaySign(payOrder,
		...
		System.out.println("PayTypeCode:"+rpPayWay.getPayTypeCode());
		payOrder.put("payWayCode", payWayCode);
		String result = rpPayChannelWechatService.doWechatRequest(rpPayWay.getPayTypeCode(), payOrder,
				rpUserPayConfig.getPaySecret());
		
		JSONObject resultObj = JSON.parseObject(result);
		String payParams = resultObj.getString("payParams");
		
		logger.info("微信JSAPI支付  调起支付的参数payParams={}",payParams);
